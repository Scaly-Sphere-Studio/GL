...
static void compose_frame(unsigned char** rows_dst, unsigned char** rows_src, unsigned char bop, unsigned int x, unsigned int y, unsigned int w, unsigned int h)
{
    unsigned int  i, j;
    int u, v, al;
...
static void processing_start(png_structp& png_ptr, png_infop& info_ptr, void* frame_ptr, bool hasInfo, CHUNK& chunkIHDR, std::vector<CHUNK>& chunksInfo)
{
...
static void processing_data(png_structp png_ptr, png_infop info_ptr, unsigned char* p, unsigned int size)
{
    if (!png_ptr || !info_ptr)
        return;
...
int load_apng(char const* szIn, std::vector<APNGFrame>& frames)
{
    ...
    errno_t err = fopen_s(&f, szIn, "rb");
    if (err != 0) {
        throw_exc(CONTEXT_MSG(getErrorString(err), szIn));
    }
